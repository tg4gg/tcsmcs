...
for dp in flw_producer: 
    ...
    if (tBetweenSamples <= 0.0):
        ...
        az_lst.append((dp.targetTime, (prevAz - dp.azPos)/tBetweenSamples))
        el_lst.append((dp.targetTime, (prevEl - dp.elPos)/tBetweenSamples))
    prevAz = dp.azPos
    prevEl = dp.elPos
...
plt.title("TCS-MCS Communication Analysis: {0} data from {1} VELOCITY".format(args.system, args.date))
#ax1.plot(diffTime, diffVal, "b.")
ax1.plot(azTime, azVal, "b.-")
ax1.grid(True)
ax1.tick_params("y", colors="b")
#ax1.set_ylabel("Time between executions (targetTime)", color="b")
ax1.set_ylabel("Azimuth distance/time (velocity)", color="b")
...
if True: #Plot also diffs
    ...
    elTime, azVal=zip(*el_lst)
    ax2 = ax1.twinx()
    #ax2.plot(execTime, execVal, "r.")
    ax2.plot(elTime, elVal, "r-")
