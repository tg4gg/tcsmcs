...
from swglib2 import DataRange, DataRangeCollection
from swglib2 import producer, find_event_ranges
...
def parse_args():
    ...
    print "Using arguments: [-arc {0} | -deg {1:0.5f}] -sig {2} -gap {3} -axis {4} -date {5}\n\t\t -hvpad {6} -plpad {7} -velthr {8} -day {9}".format(args.pmac_eth_arc, 
    					args.pmac_eth_deg, args.significant_ev, args.max_gap, args.axis, args.date, args.hvel_padding, args.plot_padding, args.vel_thr, args.eng_mode)
    ...
    args.error_data = os.path.join(
            ...
            'txt', 
            '{0}_{1}_mc-{2}PmacPosError_{3}export.txt'.format(args.date, SITE, args.axis, day_str)
        )
...
for i, dr in enumerate(error_ranges):
    ...
    ax2.set_ylim([-1*args.pmac_eth_arc, args.pmac_eth_arc])
    ...
    for err in errPeriods:
        ...
        whenEnd = datetime.strptime(date+err[1], "%Y-%m-%d %H:%M:%S")
        ax2.axvspan(when, whenEnd, facecolor='0.2', alpha=0.3)
