...
for group in find_event_ranges(vel_producer, vel_pred, significant=0, gap=args.max_gap):
    error_ranges.subtract(DataRange(group, padding=args.hvel_padding), is_significant)

if len(error_ranges):
    prevTSEnd = error_ranges.time_bounds.start
    for i, rng in enumerate(error_ranges):
        print "Range [{0}] {1} length: {2}, distance from previous: {3}".format(i, rng.time_bounds.start,\
                 rng.period_length, rng.time_bounds.start-prevTSEnd)
...
for i, dr in enumerate(error_ranges):
    ...
    plt.title("PMAC Position Error vs Velocity on Axis: %s - Error range [%d]\n%s" % (args.axis, i, dr) )
